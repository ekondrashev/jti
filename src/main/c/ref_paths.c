...
/* Every JVMTI interface returns an error
 * code, which should be checked * to
 * avoid any cascading errors down the
 * line. * The interface GetErrorName()
 * returns the actual enumeration constant
 * * name, making the error messages much
...
} /* Enter a critical section by doing a
 * JVMTI Raw Monitor Enter */
static void enter_critical_section(jvmtiEnv * jvmti) {
	jvmtiError error;
	error = jvmti->RawMonitorEnter(gdata->lock);
	check_jvmti_error(jvmti, error, "Cannot enter with raw monitor");
} /* Exit a critical section by doing a
 * JVMTI Raw Monitor Exit */
...
heap_root_callback(jvmtiHeapRootKind root_kind, jlong class_tag, jlong size,
		jlong * tag_ptr, void *user_data) {
	return JVMTI_ITERATION_CONTINUE;
}
static jvmtiIterationControl JNICALL
stack_ref_callback(jvmtiHeapRootKind root_kind, jlong class_tag, jlong size,
...
object_ref_callback(jvmtiObjectReferenceKind reference_kind, jlong class_tag,
	...
	// if tag is not present, then create ObjectInfo and set it as it's tag.
		...
		ObjectInfo(); memset(obj_info,
				0 , sizeof(ObjectInfo));
		obj_info->size = size;
		obj_info->visited = 1;
		obj_info->kind =
		reference_kind; *tag_ptr =
		(jlong)(ptrdiff_t)(void*)obj_info;
		//Add the new ObjectInfo to ObjectInfo's list
		if (objList
				== NULL) {objList = new
			ObjectInfoList(); objList->obj
			= obj_info; objList->next =
			NULL;} else {ObjectInfoList*
			list = objList; while
			(list->next != NULL) {list =
				list->next;}ObjectInfoList*
			objinfo = new ObjectInfoList();
			objinfo->obj = obj_info;
			objinfo->next = NULL;
			list->next = objinfo;}}if (referrer_tag != NULL) {
		if (obj_info->referrers == NULL) {
			obj_info->referrers = new
			Referrer();
...
JNICALL heap_object_callback(jlong class_tag, jlong size, jlong* tag_ptr,
		void *user_data) {
	static jlong i = 1;
...
void visit(ObjectInfo* object) {
	...
	jint count = 0;
	jobject* obj;
	jlong* tag_result_ptr = new
	jlong;
	JNIEnv* env;
	jlong tag = (jlong) (ptrdiff_t) object;
	err = gdata->jvmti->GetObjectsWithTags(1, (jlong*) &tag, &count, &obj,
			&tag_result_ptr);
	if (obj != NULL) { //object->obj = *obj;
		(gdata->jvm)->GetEnv((void**) &env, JNI_VERSION_1_2);
		jmethodID mid = env->GetMethodID(gdata->klass, "toString",
				"()Ljava/lang/String;");
		jstring jstr = (jstring) env->CallObjectMethod(*obj, mid); //object->jstr = jstr;
		char* str = (char*) env->GetStringUTFChars(jstr, NULL); //object->classname = str;
		printf("\n- ");
		switch (object->kind) {
...
void dfsPrintRefPaths(ObjectInfo* object) {
	visit(object); //addto(parentlist, object);
	if (parentList == NULL) {
		parentList = new
		ObjectInfoList();
	...
	enter_critical_section(jvmti);
	{
		if (!gdata->dumpInProgress) {
			...
			jvmti->IterateOverReachableObjects(&heap_root_callback,
					&stack_ref_callback, &object_ref_callback, user_data); // print ref paths
			RefPaths* list = ref_paths;
...
//----------------------------------------------------------------------------
// VM init callback static void
JNICALL callbackVMInit(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread) {
	enter_critical_section(jvmti);
	{
		jvmtiError error;
...
/* Get a token from a string
 (strtok is not MT-safe) * str
 String to scan * seps
 Separation characters * buf
 Place to put results * max Size
 of buf * Returns NULL if no
 token available or can't do the
 scan. */
...
/* Send message to
 stderr or whatever the error
 output location is and exit */
...
/* Send
 message to stdout or whatever
 the data output location is */
...
static void parse_agent_options(char *options) {
	...
	gdata->max_count = 10;
	/* Default max=n *//* Parse
	 options and set flags in gdata
	 */if (options == NULL) {
		return;
	} /* Get the first
	 token from the options string.
	 */
	next = get_token(options, ",=", token, sizeof(token)); /*
	 While not at the end of the
	 options string, process this
	 option. */
	while (next != NULL) {
		if (strcmp(token, "help") == 0) {
			...
			stdout_message(
					"\tclassname=item\t\t Print ref paths of instances of this class\n");
			stdout_message("\n");
			...
			next = get_token(next, ",=", number, (int) sizeof(number)); /* Check
			 for token scan error */
			if (next == NULL) {
				fatal_error("ERROR: max=noption error\n");
			} /* Save
			 numeric value */
			...
			if (gdata->classname == NULL) {
				...
				gdata->classname = (char*) realloc((void*) gdata->classname,
						used + maxlen + 1);
			}
			if (gdata->classname == NULL) {
			 ...
			 */if (next == NULL) {
				fatal_error("ERROR: includeoption error\n");
			}
		} else if (token[0] != 0) {
			/* We got a
			 non-empty token and we don't
			 know what it is. */
			fatal_error("ERROR: Unknown option: %s\n", token);
		}
		/* Get
		 the next token (returns NULL if
		 there are no more) */
...
Agent_OnLoad(JavaVM *jvm, char *options, void *reserved) {
	...
	/* Setup initial global agent
	 data area * Use of
	 static/extern data should be
	 handled carefully here. * We
	 need to make sure that we are
	 able to cleanup after ourselves
	 * so anything allocated in this
	 library needs to be freed in *
	 the Agent_OnUnload() function.
	 */(void) memset((void*) &data, 0, sizeof(data));
	gdata = &data; /* We need to first get
	 the jvmtiEnv* or JVMTI
	 environment */
	gdata->jvm = jvm;
	res = jvm->GetEnv((void **) &jvmti, JVMTI_VERSION_1_0);
	if (res != JNI_OK || jvmti == NULL) { /* This means that the
	 VM was unable to obtain this
	 version of the * JVMTI
	 interface, this is a fatal
	 error. */
		printf(
				"ERROR: Unable to access JVMTI Version 1(0x%x)," " is your J2SE a 1.5or newer version?" " JNIEnv'sGetEnv() returned %d\n",
				JVMTI_VERSION_1, res);
	} /*
	 Here we save the jvmtiEnv* for
	 Agent_OnUnload(). */
	gdata->jvmti = jvmti; /* Parse
	 any options supplied on java
	 command line */
	...
	/* At first the only initial
	 events we are interested in are
	 VM * initialization, VM death,
	 and Class File Loads. * Once
	 the VM is initialized we will
	 request more events. */error = jvmti->SetEventNotificationMode(JVMTI_ENABLE,
	...
	check_jvmti_error(jvmti, error, "Cannot set event notification"); /* Here we
	 create a raw monitor for our
	 use in this agent to * protect
	 critical sections of code. */
	error = jvmti->CreateRawMonitor("agentdata", &(gdata->lock));
	check_jvmti_error(jvmti, error, "Cannot create raw monitor");
	/* We return JNI_OK to signify
	 success */return JNI_OK;
}
/*
 Agent_OnUnload: This is called
 immediately before the shared
 library is * unloaded. This is
 the last code executed. */
JNIEXPORT void JNICALL Agent_OnUnload(JavaVM *vm) {
	/*
	 Make sure all
	 malloc/calloc/strdup space is
	 freed */
}
